/*
 * math/aabb2i.h - 2D integer axis-aligned bounding box (rc_aabb2i).
 *
 ...
 * Type
 * ----
 *   rc_aabb2i  { rc_vec2i min; rc_vec2i max; }
 *
 * Construction
 * ------------
 *   rc_aabb2i_make(a, b)                 — from two corner points; sorts components
 *   rc_aabb2i_make_with_margin(a, b, m)  — same, then expands each side by m;
 *                                          asserts no int32_t overflow
 *
 * Queries
 * -------
 *   rc_aabb2i_contains(a, b)       — true if a fully contains b  [a.min,a.max) ⊇ [b.min,b.max)
 *   rc_aabb2i_intersects(a, b)     — true if the two boxes overlap (touching edges do NOT count)
 *   rc_aabb2i_contains_point(a, p) — true if point p is inside a (min inclusive, max exclusive)
 *
 * Combination
 * -----------
 *   rc_aabb2i_union(a, b)          — smallest box containing both a and b
 *   rc_aabb2i_expand(a, p)         — smallest box containing a and point p
 */

#ifndef RC_MATH_AABB2I_H_
#define RC_MATH_AABB2I_H_
...
static inline rc_aabb2i rc_aabb2i_make(rc_vec2i a, rc_vec2i b)
{
    return (rc_aabb2i) {
 ...
 */
static inline rc_aabb2i rc_aabb2i_make_with_margin(rc_vec2i a, rc_vec2i b, int32_t margin)
{
 ...
 * as wide.
 */
static inline bool rc_aabb2i_contains(rc_aabb2i a, rc_aabb2i b)
{
 ...
 */
static inline bool rc_aabb2i_intersects(rc_aabb2i a, rc_aabb2i b)
{
 ...
 */
static inline bool rc_aabb2i_contains_point(rc_aabb2i a, rc_vec2i p)
{
...
static inline rc_aabb2i rc_aabb2i_union(rc_aabb2i a, rc_aabb2i b)
{
    return (rc_aabb2i) {
...
static inline rc_aabb2i rc_aabb2i_expand(rc_aabb2i a, rc_vec2i p)
{
    return (rc_aabb2i) {
