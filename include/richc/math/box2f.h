/*
 * math/box2f.h - 2D axis-aligned bounding box (rc_box2f).
 *
 ...
 * Type
 * ----
 *   rc_box2f  { rc_vec2f min; rc_vec2f max; }
 *
 * Construction
 * ------------
 *   rc_box2f_make(a, b)                 — from two corner points; sorts components
 *   rc_box2f_make_with_margin(a, b, m)  — same, then expands each side by m
 *
 * Queries
 * -------
 *   rc_box2f_contains(a, b)       — true if a fully contains b  [a.min,a.max) ⊇ [b.min,b.max)
