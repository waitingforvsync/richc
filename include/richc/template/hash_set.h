 ...
 *
 * State encoding: 0 = EMPTY, 1 = OCCUPIED, 2 = TOMBSTONE.
 * Only the states portion of each block is zeroed on allocation;
 * keys are only read when their corresponding state is OCCUPIED.
 *
 * Generated types
 * ---------------
...
static inline void SET_REHASH_(SET_NAME *set, uint32_t new_cap, rc_arena *a)
{
    ...
    /* Zero only the states array; keys are only read when state == 1. */
    ...
    if (set->data) {
        ...
        for (uint32_t i = 0; i < set->cap; i++) {
            if (old_states[i] != 1) continue;   /* skip EMPTY / TOMBSTONE */
            uint32_t j = (uint32_t)SET_HASH(old_keys[i]) & mask;
            while (new_states[j] == 1) j = (j + 1) & mask;
            new_states[j] = 1;
...
static inline int SET_ADD_(SET_NAME *set, SET_KEY_T key, rc_arena *a)
{
    ...
    for (;;) {
        ...
        for (;;) {
            uint8_t s = states[i];
            if (s == 1 && SET_EQUAL(keys[i], key))
                return 0;              /* already present */
            if (s == 2 && tomb == set->cap) tomb = i;
            if (s == 0) break;         /* EMPTY: key not in set */
            i = (i + 1) & mask;
        }
...
static inline int SET_REMOVE_(SET_NAME *set, SET_KEY_T key)
{
    ...
    for (;;) {
        uint8_t s = states[i];
        if (s == 0) return 0;   /* EMPTY: not in set */
        if (s == 1 && SET_EQUAL(keys[i], key)) {
            states[i] = 2;      /* mark TOMBSTONE */
            set->count--;
            return 1;
        }
        i = (i + 1) & mask;
...
static inline int SET_CONTAINS_(SET_NAME *set, SET_KEY_T key)
{
    ...
    for (;;) {
        uint8_t s = states[i];
        if (s == 0) return 0;
        if (s == 1 && SET_EQUAL(keys[i], key)) return 1;
        i = (i + 1) & mask;
    }
}
