 ...
 *
 * reserved records the total reservation size; it is needed to release the
 * mapping on POSIX (munmap requires the original size). On Windows the
 * release call ignores the size.
 */
typedef struct rc_arena {
    ...
    uint32_t  reserved;  /* total reserved bytes; needed to release the mapping on POSIX */
} rc_arena;
 ...
 * This is the one place arena setup may fail and be handled by the caller;
 * allocation from an established arena never fails (see below). */
 ...
 * must have space (running out is an RC_PANIC).  NEVER returns NULL - callers
 * must not check the result for NULL. */
 ...
 * ptr must be a live pointer into the arena. */
bool      rc_arena_free(rc_arena *a, void *ptr, uint32_t size);

/* Resize the allocation at ptr.  New bytes when growing are NOT zeroed.
 * new_size must be non-zero (asserted); running out of space is an RC_PANIC.
 * Never returns NULL.  See rc_arena_realloc_zero for a zeroing variant. */
