/*
 * math/mat44f.h - 4x4 single-precision float matrix (rc_mat44f).
 *
...
/* 3D translation (upper-left 3x3 is identity; last column is (v, 1)). */
...
/* Embed a 2x2 matrix into the upper-left corner (identity elsewhere). */
...
/* Embed a 3x3 matrix into the upper-left corner (w column/row = unit). */
...
{
    return (rc_mat44f) {
        rc_vec4f_from_vec3f(m.rot.cx, 0.0f),
        rc_vec4f_from_vec3f(m.rot.cy, 0.0f),
        rc_vec4f_from_vec3f(m.rot.cz, 0.0f),
        rc_vec4f_from_vec3f(m.trans,  1.0f)
    };
}
