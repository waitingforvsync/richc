 ...
 *   rc_vec3i_from_vec2i(v, z)  - extend a 2D integer vector
 *
 * Conversion
 * ----------
 *   rc_vec3i_as_i32s(a)        - pointer to the first component (int32_t[3])
 *
 * Arithmetic
 * ----------
 ...
 *   dot, lengthsqr
 *
 * Vector result with a range assertion
 * ------------------------------------
...
/*
 * Cross product of a and b.  Each component is computed in int64_t (products of
 * two int32_t values never overflow int64_t, nor does the subtraction); asserts
 * that each result component fits in int32_t before returning.
 */
